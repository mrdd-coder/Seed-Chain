      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/" className="flex items-center gap-2 font-bold text-xl tracking-tight text-slate-900 dark:text-white">
            <span className="h-6 w-6 rounded-full bg-emerald-600 flex items-center justify-center text-white text-xs">SC</span>
            <span>Seed<span className="text-emerald-600">Chain</span></span>
          </Link>
        </div>
            ...
            return (
              <Link
                ...
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  isActive
                    ? 'bg-slate-100 text-emerald-600 dark:bg-slate-800 dark:text-emerald-500'
        ...
        <div className="flex items-center gap-3">
          ...
          {isConnected && address ? (
            <div className="flex items-center gap-2 border border-slate-200 dark:border-slate-800 rounded-lg p-1 pr-3 bg-slate-50 dark:bg-slate-900">
              <div className="h-7 w-7 rounded-md bg-emerald-100 dark:bg-emerald-950 flex items-center justify-center text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase">
                {walletId ? walletId[0] : 'W'}
              </div>
              <span className="text-xs font-medium font-mono text-slate-600 dark:text-slate-300">
                {address.substring(0, 5)}...{address.substring(address.length - 4)}
              </span>
              <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-100 dark:border-emerald-900/40">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold shadow-sm transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
            >
              Connect Wallet
            </button>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-6 space-y-3">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-lg bg-emerald-100 dark:bg-emerald-950 flex items-center justify-center text-lg">
                    🚢
                  </div>
                  <div>
                    <div className="font-bold text-sm text-slate-800 dark:text-slate-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
                      Freighter Wallet
                    </div>
